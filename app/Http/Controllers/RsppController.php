...
class RsppController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $data = $militari->map(function ($militare) {
            ...
            return [
                'militare' => $militare,
                'lavoratore_4h' => $this->calcolaScadenza($scadenza?->lavoratore_4h_data_conseguimento, 5), // 5 anni
                'lavoratore_8h' => $this->calcolaScadenza($scadenza?->lavoratore_8h_data_conseguimento, 5), // 5 anni
                'preposto' => $this->calcolaScadenza($scadenza?->preposto_data_conseguimento, 2), // 2 anni
                'dirigente' => $this->calcolaScadenza($scadenza?->dirigenti_data_conseguimento, 2), // 2 anni
                'antincendio' => $this->calcolaScadenza($scadenza?->antincendio_data_conseguimento, 1), // 1 anno
                'blsd' => $this->calcolaScadenza($scadenza?->blsd_data_conseguimento, 1), // 1 anno
    ...
    public function updateSingola(Request $request, Militare $militare)
    {
        ...
        $anni = match($campo) {
            'lavoratore_4h_data_conseguimento', 'lavoratore_8h_data_conseguimento' => 5,
            'preposto_data_conseguimento', 'primo_soccorso_aziendale_data_conseguimento' => 2,
            'dirigenti_data_conseguimento', 'antincendio_data_conseguimento', 'blsd_data_conseguimento' => 1,
            default => 1
        };
