                                    ...
                                    @php
                                        ...
                                        if ($pianificazione) {
                                            if ($pianificazione->tipoServizio) {
                                                ...
                                                // Vecchio codice rimosso - ora usa la mappa sopra
...
@endpush
    ...
    const giorno = giornoEl ? giornoEl.value : '';
...
document.addEventListener('DOMContentLoaded', function() {
    // Inizializza lo scroll sincronizzato della tabella
    syncTableScroll();
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            rows.forEach(row => {
                const grado = row.cells[0] ? row.cells[0].textContent.toLowerCase() : '';
                const cognome = row.cells[1] ? row.cells[1].textContent.toLowerCase() : '';
                const nome = row.cells[2] ? row.cells[2].textContent.toLowerCase() : '';
    ...
    if (toggleWeekendsBtn) {
        toggleWeekendsBtn.addEventListener('click', function() {
            ...
            const isHidden = weekendColumns[0] ? weekendColumns[0].style.display === 'none' : false;
